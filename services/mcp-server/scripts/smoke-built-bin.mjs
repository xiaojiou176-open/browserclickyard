...
assert.equal(
  packageJson.bin?.["pagestress-mcp"],
  "./dist/server.js",
  "package smoke expects pagestress-mcp to resolve to ./dist/server.js",
);
...
try {
  ...
  assert.equal(
    child.exitCode,
    null,
    `pagestress-mcp should remain alive after startup, got exitCode=${child.exitCode}, stderr=${stderr}`,
  );
