...
const requestHeaders = automationToken
  ? {
      ...
      "x-automation-client-id": "frontend-e2e-nonstub-health-readiness",
    }
  : {};
...
test("@frontend-nonstub @nonstub health endpoint is reachable without route stubs", async ({
  page,
}) => {
  ...
  if (!response.ok()) {
    ...
    throw new Error(
      dependencyMessage(`/health returned ${response.status()}: ${body.slice(0, 300)}`),
    );
  }
